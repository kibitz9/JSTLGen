 ...
 * @author cmiller
 */
public class SDFOperationSectorDuplicateYAxis extends SignedDistanceField3d {
    ...
    public SDFOperationSectorDuplicateYAxis( SignedDistanceField3d toRotate, double sectorCount){
    ...
    public double GetRawDistance(Vector3d p) {
        double sectorNumber = Math.round(Math.atan2(p.x,p.z)/sectorSize);
        double angleOffset =-sectorNumber*sectorSize;
        double cos1 = Math.cos(angleOffset);
        double sin1 = Math.sin(angleOffset);
        Vector3d q = new Vector3d(p.x*cos1+p.z*sin1,p.y,p.z*cos1-p.x*sin1);
        return toRotate.GetDistance(q);
    }

    @Override
    public SignedDistanceField3d Clone() {
        return new SDFOperationSectorDuplicateYAxis(  toRotate, sectorCount);
    }
    ...
    public ShaderString toShaderString(String parm){
       ...
       // String qb = ShaderString.nextVariableName("qb");
        ...
        d+="\r\n\tfloat "+sectorNumberS+"=round(atan(<parm>.x,<parm>.z)/"+sectorSizeS+");";
        ...
        d+="\r\n\tvec3 "+q+"=vec3(";
        d+="\r\n\t\t<parm>.x*"+cos+"+<parm>.z*"+sin+",";
        d+="\r\n\t<parm>.y,";
        d+="\r\n\t\t<parm>.z*"+cos+"-<parm>.x*"+sin;
